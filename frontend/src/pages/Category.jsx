...
const PostCard = ({ post, variant = 'default' }) => {
    ...
    return (
        <div className={`bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-102 transition-transform duration-300 ${variant === 'small' ? 'flex items-start mb-4' : 'block'}`}>
            ...
            <div className={`p-4 ${variant === 'small' ? 'flex-grow w-3/5' : 'p-6'}`}>
                <span className={`text-sm text-gray-500 mb-2 block font-medium ${variant === 'small' ? 'text-xs' : ''}`}>
                    {post.createdAt ? format(new Date(post.createdAt), 'MMMM dd,yyyy') : 'Date Unknown'}
                </span>
                <h3 className={`font-bold text-gray-900 mb-2 line-clamp-2 ${variant === 'small' ? 'text-md' : 'text-xl'}`}>
                    <Link to={`/blog/${post._id}`} className="hover:text-blue-600 transition duration-200">
                        {post.title}
                    </Link>
                </h3>
                ...
                <Link
                    to={`/blog/${post._id}`}
                    className={`text-blue-600 hover:underline font-semibold flex items-center group ${variant === 'small' ? 'text-sm' : ''}`}
                >
                    Read Article
...
export default function CategoryPage() {
    ...
    useEffect(() => {
        ...
        if (categorySlug) {
            // Add authFetch to the dependency array
            fetchCategoryPosts();
        }
    }, [categorySlug, authFetch]); // <--- IMPORTANT: Add authFetch to dependency array

    if (loading) {
        return (
            <div className="flex justify-center items-center min-h-[calc(100vh-160px)]">
                <p className="text-xl text-gray-600 animate-pulse">Loading {categoryName || categorySlug} articles...</p>
            </div>
        );
    }
    ...
    return (
        <div className="container mx-auto p-4 max-w-7xl">
            <Helmet>
                <title>{displayCategoryTitle} - MyBlog Categories</title>
            ...
            <SectionContainer title={`${displayCategoryTitle} Articles`}>
                {posts.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                        <p className="text-xl text-gray-600">No articles found in the "{displayCategoryTitle}" category yet.</p>
                        <p className="text-md text-gray-500 mt-2">Check back soon or explore other categories!</p>
                        <Link to="/" className="inline-block mt-6 px-6 py-3 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition duration-300">
                            Go to Homepage
                        </Link>
                    </div>
                )}
